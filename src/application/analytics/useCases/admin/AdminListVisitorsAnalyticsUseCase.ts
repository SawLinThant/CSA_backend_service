...
function getRangeParams(range: VisitorsRange): { from: Date; to: Date; mode: VisitorsBinMode } {
  const to = new Date();
...
export class AdminListVisitorsAnalyticsUseCase {
  constructor(private readonly analyticsRepository: AnalyticsRepository) {}

  async execute(query: VisitorsRangeQueryInput): Promise<VisitorsSeriesPoint[]> {
    const { range } = query;
