...
export class CustomerCreateSubscriptionUseCase {
  ...
  async execute(userId: string, input: CreateSubscriptionInput) {
    ...
    logger.info('Subscribe+reserve started', {
      userId,
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (existing?.subscription) {
          logger.info('Subscribe+reserve idempotency hit', {
            customerId: customer.id,
            planId: input.planId,
            cycleDate: cycleDate.toISOString(),
        ...
        if (!hasAddress) {
          logger.error('Subscribe+reserve rejected: no delivery address', {
            userId,
            customerId: customer.id,
            planId: input.planId,
          });
        ...
        if (activeSubscription) {
          logger.error('Subscribe+reserve rejected: active subscription already exists', {
            customerId: customer.id,
        ...
        if (!boxVersion) {
          logger.error('Subscribe+reserve failed: no active box version', {
            customerId: customer.id,
            planId: input.planId,
            boxId: plan.boxId,
            cycleDate: cycleDate.toISOString(),
        ...
        if (!snapshot) {
          ...
          logger.info('Capacity snapshot computed in subscribe flow', {
            customerId: customer.id,
            planId: input.planId,
        ...
        const lockedRows = await tx.$queryRaw<
          Array<{ id: string; max_boxes: number; reserved_boxes: number; consumed_boxes: number; status: string }>
        >`SELECT id, max_boxes, reserved_boxes, consumed_boxes, status
           FROM capacity_snapshots
           WHERE id = ${snapshot.id}
           FOR UPDATE`;
        const locked = lockedRows[0];
        if (!locked) throw new Error('Capacity snapshot not found');
        if (locked.status !== 'open') {
          logger.error('Subscribe+reserve rejected: snapshot not open', {
            customerId: customer.id,
            planId: input.planId,
            boxVersionId: boxVersion.id,
            cycleDate: cycleDate.toISOString(),
        ...
        if (remaining < 1) {
          logger.error('Subscribe+reserve rejected: capacity exhausted', {
            customerId: customer.id,
            planId: input.planId,
            boxVersionId: boxVersion.id,
            cycleDate: cycleDate.toISOString(),
        ...
        logger.info('Subscribe+reserve committed', {
          customerId: customer.id,
          planId: input.planId,
          boxVersionId: boxVersion.id,
          cycleDate: cycleDate.toISOString(),
      ...
      logger.error('Subscribe+reserve failed', {
        userId,
        customerId: customer.id,
        planId: input.planId,
        cycleDate: cycleDate.toISOString(),
